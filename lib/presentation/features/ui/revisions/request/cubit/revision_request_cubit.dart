...
class RevisionRequestCubit extends Cubit<RevisionRequestState> {
  ...
  void selectCategory(RevisionCategory category) => emit(
    state.copyWith(selectedCategory: () => category, categoryError: () => null),
  );
  ...
  void removeAttachment(String path) => emit(
    state.copyWith(
      attachmentPaths: state.attachmentPaths.where((p) => p != path).toList(),
    ),
  );
